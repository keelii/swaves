  ...
  var NodeType = class _NodeType {
    ...
    */
    create(attrs2 = null, content, marks) {
      ...
      return new Node(this, this.computeAttrs(attrs2), Fragment.from(content), Mark.setFrom(marks));
    }
    /**
    ...
    */
    createChecked(attrs2 = null, content, marks) {
      ...
      return new Node(this, this.computeAttrs(attrs2), content, Mark.setFrom(marks));
    }
    /**
    ...
    */
    createAndFill(attrs2 = null, content, marks) {
      ...
      if (!after)
        return null;
      return new Node(this, attrs2, content.append(after), Mark.setFrom(marks));
    }
    /**
...
`;
  ...
  function createMermaidPreviewNodeView(node) {
    ...
    function render(nextNode) {
      ...
      loadMermaidPreviewRuntime().then(function() {
        ...
        diagram.textContent = error2 && error2.message ? error2.message : "Mermaid render failed.";
        button.hidden = true;
      });
    }
    button.addEventListener("click", function(event) {
  ...
  function isMathRawBlockText(text2) {
    var lines = normalizeNewlines(text2).split("\n");
    if (lines.length < 2) {
      return false;
    }
    return lines[0].trim() === "$$" && lines[lines.length - 1].trim() === "$$";
  }
  function parseMathRawInlineSource(text2) {
    var source = String(text2 == null ? "" : text2).trim();
  ...
  function extractMathRawBlockExpression(text2) {
    var lines = normalizeNewlines(text2).split("\n");
    if (lines.length < 2) {
      return "";
    }
  ...
  function isIframeRawBlockText(text2) {
    var trimmed = normalizeNewlines(text2).trim();
    if (!trimmed) {
      return false;
    }
    return /^<iframe\b/i.test(trimmed) && /<\/iframe>\s*$/i.test(trimmed);
  }
  function getRawBlockPreviewKind(text2) {
    if (isMathRawBlockText(text2)) {
      return "math";
    }
    if (isIframeRawBlockText(text2)) {
      return "iframe";
    }
    return "";
  }
  ...
  function parseIframeRawBlockSource(text2) {
    if (typeof document === "undefined") {
      return null;
    }
    var source = normalizeNewlines(text2).trim();
  ...
  function createRawBlockPreviewNodeView(node) {
    ...
    function renderMath(nextSource, seq) {
      ...
      loadKatexPreviewRuntime().then(function(katex) {
        ...
        renderError(error2 && error2.message ? error2.message : "KaTeX render failed.");
      });
    }
  ...
  function splitRawInlineText(text2) {
    ...
    while (match2 = pattern.exec(input)) {
      if (match2.index > last) {
        out.push({ kind: "text", text: input.slice(last, match2.index) });
      }
      out.push({ kind: "raw_inline", text: match2[0] });
  ...
  function parseMarkdown(schema2, parser, markdown) {
    ...
    for (var idx = 0; idx < segments.length; idx += 1) {
      ...
      if (seg.kind === "raw_block") {
        ...
        var rawContent = rawText ? schema2.text(rawText) : null;
    ...
    if (blocks.length === 0) {
      blocks.push(schema2.nodes.paragraph.create());
    }
  ...
  function buildPlugins(schema2, options) {
    ...
    if (schema2.nodes.heading && schema2.nodes.paragraph) {
      headingRule = new InputRule(/^(#{1,6})\s$/, function(state, match2, start, end) {
        ...
        if ($start.parent.type !== schema2.nodes.paragraph) {
          return null;
        }
        var level = match2[1].length;
        if (level < 1 || level > 6) {
          return null;
        }
        var headingType = schema2.nodes.heading;
        ...
        return state.tr.delete(start, end).setBlockType(start, start, headingType, { level });
      });
    }
    var inputRuleList = [];
    if (headingRule) {
    ...
    return [
      ...
      keymap({
        ...
        Enter: splitListItemCommand || void 0,
    ...
    view = new EditorView(mount, {
      ...
      nodeViews: {
        ...
        raw_inline: function(node) {
          ...
          return createRawInlinePreviewNodeView(node) || void 0;
        },
        raw_block: function(node) {
          if (!opts.rawBlockPreview) {
            return void 0;
          }
          return createRawBlockPreviewNodeView(node) || void 0;
        }
      },
