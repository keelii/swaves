...
func TestReadRuntimeInfoFallsBackWhenCurrentPathIsInvalid(t *testing.T) {
	base := t.TempDir()
	withUpdaterWorkingDir(t, base)
	...
	currentPath := filepath.Join(base, ".cache", "updater", "master_runtime.json")
	if err := os.MkdirAll(filepath.Dir(currentPath), 0755); err != nil {
		t.Fatalf("MkdirAll current path failed: %v", err)
	}
	if err := os.WriteFile(currentPath, []byte(`{"pid":`), 0644); err != nil {
		t.Fatalf("WriteFile current path failed: %v", err)
	}

	legacyPath := filepath.Join(base, ".cache", "swaves", "master_runtime.json")
	if err := os.MkdirAll(filepath.Dir(legacyPath), 0755); err != nil {
		t.Fatalf("MkdirAll legacy path failed: %v", err)
	}
	if err := os.WriteFile(legacyPath, []byte(`{"pid":3456,"executable":"/root/swaves"}`), 0644); err != nil {
		t.Fatalf("WriteFile legacy path failed: %v", err)
	}
