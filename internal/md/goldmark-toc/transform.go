...
func newRawHTML(src *[]byte, s string) *ast.RawHTML {
	start := len(*src)
	*src = append(*src, []byte(s)...)
	stop := len(*src)

	n := ast.NewRawHTML()
	n.Segments.Append(text.NewSegment(start, stop))
	return n
}
...
func (t *Transformer) Transform(doc *ast.Document, reader text.Reader, ctx parser.Context) {
	...
	//<div onclick="this.parentNode.classList.toggle('show')" class="toggle">§</div>
	openTag := newRawHTML(&src, `<div id="toc" class="show">`)
	closeTag := newRawHTML(&src, `</div>`)
	first := doc.FirstChild()

	doc.InsertBefore(doc, first, openTag)
	doc.InsertAfter(doc, openTag, heading)
	doc.InsertAfter(doc, heading, listNode)
	doc.InsertAfter(doc, listNode, closeTag)
