...
func RegisterRouter(app *fiber.App, gStore *store.GlobalStore) {
	...
	dashGroup.Get("/themes", handler.GetThemeListHandler).Name("dash.themes.list")
	dashGroup.Get("/themes/default", handler.GetDefaultThemeEntryHandler).Name("dash.themes.default")
	dashGroup.Post("/themes/import", handler.PostImportThemeHandler).Name("dash.themes.import")
	dashGroup.Get("/themes/new", handler.GetThemeNewHandler).Name("dash.themes.new")
	dashGroup.Post("/themes", handler.PostCreateThemeHandler).Name("dash.themes.create")
	dashGroup.Get("/themes/:id", handler.GetThemeEditHandler).Name("dash.themes.edit")
	dashGroup.Get("/themes/:id/export", handler.GetExportThemeHandler).Name("dash.themes.export")
	dashGroup.Post("/themes/:id", handler.PostUpdateThemeHandler).Name("dash.themes.update")
	dashGroup.Post("/themes/:id/copy", handler.PostCopyThemeHandler).Name("dash.themes.copy")
	dashGroup.Post("/themes/:id/delete", handler.PostDeleteThemeHandler).Name("dash.themes.delete")
	dashGroup.Post("/themes/:id/set-current", handler.PostSetCurrentThemeHandler).Name("dash.themes.set_current")
	...
	dashGroup.Post("/api/trash/:type/batch-restore", handler.PostTrashBatchRestoreAPIHandler).Name("dash.trash.api.batch_restore")
	...
	dashGroup.Post("/backup-restore/remote-backup", handler.PostBackupRestoreRemoteBackupNowHandler).Name("dash.backup_restore.remote_backup")
